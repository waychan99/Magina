...
static const CGFloat CSToastVerticalPadding     = 13.0;
...
{
    for (UIView *atoast in self.subviews)
    {
        if (atoast.exclusiveTouch )
        {
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    // sanity
    if((message == nil || [message isKindOfClass:[NSNull class]]) && (title == nil || [title isKindOfClass:[NSNull class]]) && (image == nil)) return nil;
    ...
    wrapperView.backgroundColor = [UIColor colorWithRed:17/255.0 green:17/255.0 blue:17/255.0 alpha:0.7];
